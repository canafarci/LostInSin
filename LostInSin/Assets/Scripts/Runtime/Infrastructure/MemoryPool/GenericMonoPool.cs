...
namespace LostInSin.Runtime.Infrastructure.MemoryPool
{
	public class GenericMonoPool<T> where T : MonoBehaviour, IPoolable
	{
		...
		private List<T> _initialObjectList = new();
		...
		private void OnAppStateChangedHandler(AppStateID newState, AppStateID oldState)
		{
			if (oldState == _appStateToRecycle)
				DestroyAll();
			else if (newState == _appStateToRecycle) InstantiateDefaultObjects();
		}
		...
		{
			foreach (T mono in GetInternalList()) OnDestroyMono(mono);
		}

		private T OnCreate()
		{
			T mono = Object.Instantiate(_prefab).GetComponent<T>();
			mono.OnCreate();

			if (!_recycleWithSceneChange) Object.DontDestroyOnLoad(mono);
		...
		~GenericMonoPool()
		{
			...
			if (_recycleWithSceneChange) PoolManager.OnAppStateChanged -= OnAppStateChangedHandler;
		}
	}
}